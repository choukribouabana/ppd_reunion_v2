...
export default class modifUser extends React.Component{
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    const mod = JSON.parse(localStorage.getItem('util'));
    this.setState({ mod });
    const val = true;
    this.setState({ val });
    //console.log(mod);
...
<div className="container-fluid p-5  
     ...
     userService.modifieuser(this.state.mod._id,values).then( (response)=> {
       ...
       .catch(error => {
        ...
        // actions.isSubmitting= false;
          ...
          initialValues={{name: this.state.mod.name, email: this.state.mod.email, password: '',username: this.state.mod.username, poste:this.state.mod.poste,prenom:this.state.mod.prenom,date: null, roles : null ,Ajoutimage:undefined}}
        ...
        >
          ...
          touched}) => (
            ...
            <div className="h3 d-flex justify-content-center"> Modifier utilisateur </div>
